...
// duration float64 — длительность тренировки в часах.
func RunningSpentCalories(action int, weight, duration float64) (spentCalories float64) {
	spentCalories = runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift
	spentCalories *= weight / mInKm * duration * minInH
	return
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) (spentCalories float64) {
	speed := meanSpeed(action, duration) * kmhInMsec
	spentCalories = walkingCaloriesWeightMultiplier * weight
	spentCalories += math.Pow(speed, 2) / (height / cmInM) * walkingSpeedHeightMultiplier * weight
	spentCalories *= duration * minInH
	return
}
...
// duration float64 — длительность тренировки в часах.
// weight float64 — вес пользователя.
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) (spentCalories float64) {
	speed := swimmingMeanSpeed(lengthPool, countPool, duration)
	spentCalories = (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return
}
